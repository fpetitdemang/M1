...
;- l'environnement est un SCHEME-like (la distinction defvar / defparameter expliquée en cours n'est pas gérée, car pour CLISP, defparameter et defvar ne sont que 2 formes syntaxiques différentes)
...
(mdefun meval (expr &optional (ensvar ensvar) (ensfct ensfct))
	...
	(if (consp expr)
	    (cond
	     ...
	     ; * cas par défaut: on suppose que la tête de liste est une fonction CLISP et on va évaluer tous ses arguments avant de passer la main à l'évaluateur CLISP
	     (t (metafunction expr ensvar ensfct))
	     )
	  ; * si l'argument passé n'est pas une liste, on lui cherche une valeur si c'est un symbole qui n'est pas un mot-clef en donnant la priorité à l'environnement METAEVAL et si on ne trouve pas de valeur on va chercher dans l'environnement CLISP
	  (if (keywordp expr)
	      expr
	    (if (symbolp expr)
...
(mdefun metaevallabels (expr ensvar ensfct)
	; ajoute une fonction locale à l'environnement, la retire de la liste du labels et recommence, jusqu'à-ce qu'il n'y en ait plus
	; on évalue alors le corps
	(labels ((aux (expr)
		      (if (atom (car expr))
...
; pourquoi traiter member? juste pour que la fonction de test que l'on peut passer soit méta-évaluée

(mdefun metaevalmember (expr ensvar ensfct)
	; applique le test à chaque élément de la liste
	(labels ((aux (a l r)
		      (if l
...
(mdefun metaevalload (expr ensvar ensfct)
	; évalue ce qu'on lit dans le fichier tant qu'on ne tombe pas sur nil
	(labels ((aux (X)
		      (let ((r (read X)))
			(if r
