...
; Eric pour son soutien moral, lors des supplices que furent les séances de déboggage et Gauthier pour son aide à la réflection

; ***** différences avec l'évaluateur CLISP *****
;- l'environnement est un SCHEME-like (la distinction defvar / defparameter expliquée en cours n'est pas gérée, car CLISP ne semble déjà pas la gérer)
;- le loop n'est géré que dans le cas du loop-while-do
...
;- (function f) = #'f -> f
;- (function (lambda ...)) = #'(lambda ...) -> (lambda ...) (modifiée)
;- (lambda ...) -> (lambda ...)
...
; ***** maintenant que l'on a la fonction mdefun, on meta-définit les autres fonctions quand c'est possible *****
...
(mdefun metaevalfunction (expr ensvar ensfct)
	(if (and (consp (car expr)) (equal 'lambda (caar expr)))
	    `(lambda ,(cadar expr) (let ,(car ensvar) ,(caddar expr)))
	    (car expr)))
