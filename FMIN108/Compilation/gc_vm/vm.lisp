...
; crée une machine virtuelle avec la mémoire demandé
(defun vm-make ( name sizeMemory )
    (progn
...
(defun vm-run (name expr DEBUG)
  (let ((pos (get_register name 'Next_Free_Pos)))
    (vmloader name expr)
    (run_vm name pos DEBUG)))

; execute une expression pré-compilé
(defun vm-exec (name expr DEBUG)
	(vm-run name expr DEBUG)
)

; execute une expression lisp
; ATTENTION: le compilateur doit être chargé
(defun vm-exec-gc (name expr DEBUG)
	(vm-run name (gc-exec expr) DEBUG)
)
