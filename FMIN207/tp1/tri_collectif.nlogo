...
to go  ;; turtle procedure
  search-for-chip
  if color-chip = green [find-pile -50 -50]
  if color-chip = yellow [find-pile 50 50] 
  put-down-chip
end

to search-for-chip  ;; turtle procedure -- "picks up chip" by turning orange
  ifelse pcolor != black
  [ set color-chip pcolor
    set pcolor black
    set color orange
   ]
  [ wiggle
